...
def test_sample(snapshot):
    ...
    model.optimize()
    snapshot.assert_match(str(model.Status), "Status.txt")
    snapshot.assert_match(str(model.ObjVal), "ObjVal.txt")
    snapshot.assert_match(str([(v.VarName, v.X) for v in model.getVars()]), "VarName.txt")
    snapshot.assert_match(str([(c.ConstrName, c.Pi) for c in model.getConstrs()]), "ConstrName.txt")
